...
public class RookManeuver(IKeypad keypad, int phoneNumberLength, IEnumerable<IStartValidationRule>? startValidationRules = null, IEnumerable<IManeuverValidationRule>? maneuverValidationRules = null)
: BaseManeuver(keypad, maneuver, phoneNumberLength, startValidationRules, maneuverValidationRules)
{
    private static readonly Maneuver maneuver = new(ManeuverType.Rook, new List<Move> {
        new Move(0, 1),
        new Move(0, -1),
        new Move(1, 0),
        new Move(-1, 0)
